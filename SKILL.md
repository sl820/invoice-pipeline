...
description: 阿里公益平台发票自动化流水线。扫描未规范命名的电子发票 PDF，用百炼 bl 多模态识别提取"交款人"字段，按公司名重命名后归档，再用 Playwright/CDP 自动按抬头匹配并上传到公益平台。
---
...
1. 扫 PDF
2. 复用 `mapping_*.json` 缓存或调 `bl` 抽交款人
...
- 查 询：`button:has-text('查 询')`
- 重 置：`button:has-text('重 置')`
- 立即开票：`table tbody tr.ant-table-row button:has(span:text-is('立即开票'))`
...
| `out\failed\upload-*.log` | 上传失败日志（match-not-found / no-pending / match-multiple / error） |
...
| `no-pending` | skip | `out\failed\upload-*.log` |
| `match-multiple` | skip | `out\failed\upload-*.log`（含候选列表） |
...
match_strategy: 1-to-1 by invoice_title (待开具 status only)
scope: 电子发票 PDF
